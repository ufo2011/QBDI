...
TEST_CASE_METHOD(OptionsTest, "OptionsTest_X86-ATTSyntax") {
    ...
    const QBDI::InstAnalysis* ana = vm.getCachedInstAnalysis(addr, QBDI::ANALYSIS_DISASSEMBLY);
    REQUIRE(ana != nullptr);
    ...
    vm.clearAllCache();
    vm.setOptions(QBDI::Options::ATT_SYNTAX);
    CHECK(vm.precacheBasicBlock(addr));
...
static QBDI::VMAction setBool(QBDI::VMInstanceRef vm, QBDI::GPRState* gprState, QBDI::FPRState* fprState, void* data) {
    *((bool*) data) = true;
    return QBDI::VMAction::CONTINUE;
}

TEST_CASE_METHOD(OptionsTest, "OptionsTest_X86-setOption") {
    // check if the callback and the instrumentation range are keep when use setOptions
    ...
    cbReach = false;
    vm.setOptions(QBDI::Options::DISABLE_FPR);
    REQUIRE(vm.call(&retval, addr, {}));
    REQUIRE(cbReach);
    REQUIRE(state->eax == 0x60);

    cbReach = false;
    vm.setOptions(QBDI::Options::DISABLE_OPTIONAL_FPR);
    REQUIRE(vm.call(&retval, addr, {}));
    REQUIRE(cbReach);
